...
//			System.out.println("hasIP입니다 : " + hasIP);
			...
			//IP가 있다면 hasIP에 ARP 캐시 테이블에 해당 IP, MAC이 있는 인덱스를 저장
				...
				System.out.println("flag : U, ARP 존재하므로 바로 Send");
				...
				//현재 port 번호와 나가야 할 port 번호가 다르다면 해당 port(interface)로 이동하여 해당 Send 호출 (다른 port에서 send)
					...
					//this.anotherPort.inputToHeader(input, dstIP);
			...
			//this.m_sHeader.ip_dst.addr = inform.get(2);
			...
			if (hasIP == -1) {
				...
				if(Byte.toUnsignedInt(inform.get(4)[0]) == this.port) {
					...
					((EthernetLayer) this.anotherPort.GetUnderLayer().GetUnderLayer()).SetEnetDstAddress(mac);
					//this.anotherPort.inputToHeader(input, inform.get(2));
					this.anotherPort.Send();
				}
			}	
			return true;
...
//		int index = this.routingTable.matchEntry(this.m_sHeader.ip_dst.addr);
//	    ArrayList<byte[]> inform = routingTable.getEntry(index);
//	      
//	    if(Byte.toUnsignedInt(inform.get(4)[0]) == this.port) {
//	    	this.Send();
...
//	    else {
//	    	((EthernetLayer) this.anotherPort.GetUnderLayer().GetUnderLayer()).SetEnetDstAddress(mac);
//	    	this.anotherPort.copyHeader(this.m_sHeader, this.m_sHeader.ip_dst.addr);
//	    	this.anotherPort.Send();
//	    }
//	}
